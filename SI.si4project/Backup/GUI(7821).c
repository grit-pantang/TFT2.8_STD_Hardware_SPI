 ...
 * @retvalue   :None
******************************************************************************/
void LCD01_ShowChar(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 size,u8 mode)
{  
    u8 temp;
    u8 pos,t;
	...
	num=num-' ';//得到偏移后的值
	LCD01_SetWindows(x,y,x+size/2-1,y+size-1);//设置单个文字显示窗口
	if(!mode) //非叠加方式
	{		
		for(pos=0;pos<size;pos++)
		{
			...
			for(t=0;t<size/2;t++)
		    {                 
		        if(temp&0x01)Lcd_WriteData01_16Bit(fc); 
	...
	}else//叠加方式
	{
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {   
				POINT_COLOR=fc;              
		        if(temp&0x01)LCD01_DrawPoint(x+t,y+pos);//画一个点    
		        temp>>=1; 
		    }
		}
	}
	...
	for (k=0;k<HZnum;k++) 
			{
			  ...
			  { 	LCD01_SetWindows(x,y,x+64-1,y+64-1);
				    for(i=0;i<64*4;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(tfont64[k].Msk[i]&(0x80>>j))	Lcd_WriteData01_16Bit(fc);
								else Lcd_WriteData01_16Bit(bc);
							}
							else
							{
								POINT_COLOR=fc;
								if(tfont64[k].Msk[i]&(0x80>>j))	LCD01_DrawPoint(x,y);//画一个点
								x++;
								if((x-x0)==64)
								{
									x=x0;
        ...
        if(!bHz)
        {
	        ...
	        {          
		        ...
		        else
				{
					...
					{  
					LCD01_ShowChar(x,y,fc,bc,*str,16,mode);
					x+=8; //字符,为全字的一半 
					}
					else
					{
					LCD01_ShowChar(x,y,fc,bc,*str,size,mode);
					x+=size/2; //字符,为全字的一半 
					}
				} 
				str++; 
