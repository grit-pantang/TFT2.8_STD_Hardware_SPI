...
void LCD01_ShowChar(u16 x, u16 y, u16 fc, u16 bc, u8 num, u8 size, u8 mode)
{
    ...
    if (!mode)                                              // 非叠加方式
    {
        for (pos = 0; pos < size; pos++)
        {
            ...
            {
                if (temp & 0x01)
                    Lcd_WriteData01_16Bit(fc);
                else
                    Lcd_WriteData01_16Bit(bc);
                temp >>= 1;
            }
        }
    }
    else // 叠加方式
    {
        for (pos = 0; pos < size; pos++)
        {
            ...
            for (t = 0; t < size / 2; t++)
            {
                POINT_COLOR = fc;
                if (temp & 0x01)
                    LCD01_DrawPoint(x + t, y + pos); // 画一个点
                temp >>= 1;
            }
        }
    }
